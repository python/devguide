...
The AST is generated from the parse tree using the function
``_PyPegen_run_parser()`` (:file:`Parser/pegen.c`), which is a thin wrapper over
the function ``_PyPegen_parse()`` (:file:`Parser/parser.c`).

The function begins applying production rules on the tokens it
receives; matching tokens recursively to their corresponding rule and calling
the corresponding rule functions which usually follow the format :samp:`xx_rule`.
Where *xx* is the grammar rule that the function handles (these names are
derived from :file:`Grammar/python.gram`).
...
ones) are memoized.  Each rule checks if a memoized version exists and returns
that if so, else it continues in the manner stated in the previous paragraph.

Macros for using ``asdl_seq *`` types as found
in :file:`Python/asdl.c` and :file:`Include/Internal/pycore_asdl.h` are as
follows:
...
There are also typed counterparts for some of the macros above.  These
carry out checks only in debug mode and aid debugging errors caused by
incorrectly casting from ``void *``.  It's recommended to use these over
the untyped versions in new code.  Internally, there are three main types
(``generic``, ``identifier`` and ``int``).  Typed functions and macros
for creating and using `asdl_seq *`` types are as follows:
...
The following macros handle ``asdl_xx_seq *`` types, where *xx* is the sequence
type as found in :file:`Include/Internal/pycore_asdl.h` and

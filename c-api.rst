...
CPython's public C API if available when ``Python.h`` is included normally.
...
- New Limited API should only be defined if ``Py_LIMITED_API`` is set
  to a the version the API was added in or higher.
...
- The limited API and its intended use must follow standard C, not just
  ...
  - Follow the C dialect specified in :pep:`7`.
...
- Add an entry to the end of the Stable ABI manifest, ``Misc/stable_abi.txt``.
